...
exports.returnBarcode = function (barcode, cb) {
  if (!barcode) return false

  MongoClient.connect(mongoConnectURL, function (err, db) {
    var collection = (exports.testOverride) ? db.collection('test') : db.collection('item')
    collection.find({barcode: parseInt(barcode)}).toArray(function (err, item) {
      if (item[0]) {
        item = item[0]
        db.collection('bib').find({_id: { $in: item.bibIds }}).toArray(function (err, bibs) {
          cb(err, {item: item, bib: bibs})
          db.close()
        })
      } else {
        item = false
